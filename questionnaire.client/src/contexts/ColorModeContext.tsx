  ...
  let theme = createTheme({
    ...
    components: {
      ...
      MuiAppBar: {
        styleOverrides: {
          colorPrimary: {
            backgroundColor: mode === "dark" ? "#515151" : undefined,
            // rely on primary.main but enforce good contrast by ensuring text color
  ...
  const [mode, setModeState] = useState<UserColorMode>(() => {
    const stored =
      typeof window !== "undefined"
        ? (localStorage.getItem(STORAGE_KEY) as UserColorMode | null)
        : null;
    return stored || "system";
  });
  const [systemMode, setSystemMode] = useState<"light" | "dark">(
    typeof window !== "undefined" ? resolveSystem() : "light"
  );
